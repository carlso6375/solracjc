...
const config = {
    A187: '📍SOLRACBOT📍',
    instagram: '@carlooooooossss____ ',
    Whatsapp : 'https://api.whatsapp.com/send?phone=5588981321692',
    ativo: '24 JAM',
       ...
       case 'ytmp3':
           ytdl('mp3', value)
               .then(data => {
                   ...
                   let hasil = `OLA AQUI ESTA O LINK DE DOWNLOAD DA MSC GAY\nCLIQUE NO LINK ABAIXO:\nMUSICA: ${judul}\n\nTamanho Do audio: ${size}\n\nLink: ${link}`
                   client.sendMessage(id, '[ESPERE UM FILHO DA PUTA...', MessageType.text)
       ...
       case 'ytmp4':
           ytdl('mp4', value)
               .then(data => {
                   const { judul, size, hasil: link } = data
                   let hasil = `OLA AQUI ESTA O LINK DE DOWNLOAD DA MSC GAY\nCLIQUE NO LINK ABAIXO:\nMUSICA: ${judul}\n\nTamanho Do audio: ${size}\n\nLink: ${link}`
                   client.sendMessage(id, '[ESPERE UM POUCO FILHO DA PUTA ...', MessageType.text)
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
